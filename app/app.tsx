...
return safeDescription.slice(0, 120) + "...";
}
...
> <Ionicons name="chevron-back" size={26} color="#FFFFFF" /> </TouchableOpacity>
    ...
    <Text className="text-teal-100 mt-2 text-sm">
  ...
  {isLoading ? (
    ...
    <View className="flex-1 items-center justify-center px-6">
      ...
      <Text className="text-gray-500 text-center mt-2">
        Published events and programs will appear here.
      </Text>
    </View>
  ) : (
    <ScrollView
    ...
    >
      {events.map((event) => {
        const imageUrl = event.banner_url || event.image_url || "";
